...
BusResponse Bus::broadcast(BusMessage message_type, uint32_t address, int sender_idx) {
    switch(message_type) {
    case ReadExclusive:
    ...
    for (int i = 0; i < memory_blocks.size(); i++) {
        ...
        if (memory_blocks[i]->process_signal_from_bus(message_type, address) == IsShared) {
            response = IsShared;
        }
    }
